...
struct connection {
	int id;	// skynet_socket fd
	uint32_t agent; //每个connection对应一个agent 的handle
	uint32_t client; //每个connection对应一个client 的handle
	char remote_name[32];
...
struct gate {
	...
	int listen_id; //监听的socket id
	uint32_t watchdog; 	//watchlog handle
...
_parm(char *msg, int sz, int command_sz) {
    //找到msg中每command_sz之后第一个非空的字符
	while (command_sz < sz) {
		if (msg[command_sz] != ' ')
			break;
		++command_sz;
	}
    //复制command_sz以后的字符到msg
	int i;
	for (i=command_sz;i<sz;i++) {
		msg[i-command_sz] = msg[i];
	}
    //设置i-command_sz的字符为尾字符，即把比之更长的字符都丢掉
	msg[i-command_sz] = '\0';
}
...
_ctrl(struct gate * g, const void * msg, int sz) {
	...
	if (memcmp(command,"forward",i)==0) {
        //取msg剩余的字符，即把command的字符串从原字串中截除
        printf("tmp = %s",tmp);
		_parm(tmp, sz, i);
		char * client = tmp;
...
start_listen(struct gate *g, char * listen_addr) {
	...
	int port;
	if (portstr == NULL) {
		port = (int)strtol(listen_addr, NULL, 10);
		if (port <= 0) {
        ...
        //port
		port = (int)strtol(portstr + 1, NULL, 10);
		if (port <= 0) {
			skynet_error(ctx, "Invalid gate address %s",listen_addr);
			return 1;
		}
