...
struct harbor {
	...
	int id;             //harbor_id
	struct hashmap * map;
	int master_fd;          //连接到master的socket fd
	char * master_addr;     // master address
...
_connect_to(struct harbor *h, const char *ipaddress, bool blocking) {
	...
	int sz = (int)(port - ipaddress);
	char tmp[sz + 1];
	memcpy(tmp,ipaddress,sz);
	tmp[sz] = '\0';

	int portid = (int)strtol(port+1, NULL,10);
...
_launch_gate(struct skynet_context * ctx, const char * local_addr) {
	...
	uint32_t gate = (uint32_t)strtoul(gate_addr+1 , NULL, 16);
	if (gate == 0) {
