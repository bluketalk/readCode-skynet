...
struct skynet_context {
	void * instance; //调用对应_create方法返回的实例
	struct skynet_module * mod; //动态.so模块
	uint32_t handle;
	int ref;
	char result[32];
	void * cb_ud;
...
skynet_context_new(const char * name, const char *param) {
	...
	ctx->mod = mod;//动态.so模块
	ctx->instance = inst; //调用create方法返回的实例
	ctx->ref = 2;
	ctx->cb = NULL;
	ctx->cb_ud = NULL;
	ctx->session_id = 0;
	...
	ctx->init = false;
	ctx->endless = false;
	ctx->handle = skynet_handle_register(ctx);//生成并注册handle
    //初始化一个消息队列
...
_dispatch_message(struct skynet_context *ctx, struct skynet_message *msg) {
	assert(ctx->init);
	CHECKCALLING_BEGIN(ctx)
	int type = (int)msg->sz >> HANDLE_REMOTE_SHIFT;
...
_filter_args(struct skynet_context * context, int type, int *session, void ** data, size_t * sz) {
    ...
    // 把 type 编码到了 size 参数的高 8 位。因为单个消息包限制长度在 1.6 M （24 bit)内
